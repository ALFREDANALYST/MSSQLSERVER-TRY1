...
UPDATE dbo.DEVICES
SET FIRMWARE_VERSION = 'v3.1.1'
WHERE FIRMWARE_VERSION = 'v3.1.0';
GO

SELECT * FROM DEVICES
WHERE FIRMWARE_VERSION = 'v3.1.1' ;

---------------TRANSACTIONS------------

BEGIN TRANSACTION;
UPDATE DEVICES SET BATTERY_LIFE_DAYS = BATTERY_LIFE_DAYS + 1 WHERE FIRMWARE_VERSION = 'v3.1.1'
UPDATE DEVICES SET BATTERY_LIFE_DAYS = BATTERY_LIFE_DAYS - 1 WHERE FIRMWARE_VERSION = 'v3.2.1'
...
BEGIN TRANSACTION;
UPDATE DEVICES SET ACTIVE_HOURS_DAILY = ACTIVE_HOURS_DAILY - 5 WHERE USER_ID = 'TH001';
-- Simulate a check: prevent negative quantity
IF (SELECT ACTIVE_HOURS_DAILY FROM DEVICES WHERE USER_ID = 'TH001') < 0
    ROLLBACK TRANSACTION;
ELSE
    COMMIT TRANSACTION;
SELECT * FROM DEVICES
...
BEGIN TRANSACTION;
UPDATE DEVICES
SET ACTIVE_HOURS_DAILY = ACTIVE_HOURS_DAILY + 10
WHERE USER_ID = 'TH002';
COMMIT TRANSACTION;
...
-- Lock the row for location update
SELECT city, country
FROM [dbo].[Customers] WITH (ROWLOCK, UPDLOCK)
WHERE user_id = 'TH002';

-- Update the user_id's city and country.
...
EXEC sp_columns @table_name = 'Devices';
EXEC sp_columns @table_name = 'HealthMetrics';
EXEC sp_columns @table_name = 'Sales';

EXEC sp_helpconstraint 'dbo.Customers';
EXEC sp_helpconstraint 'dbo.Devices';
EXEC sp_helpconstraint 'dbo.HealthMetrics';
EXEC sp_helpconstraint 'dbo.Sales';

SELECT C.* FROM CUSTOMERS AS C
SELECT D.* FROM DEVICES AS D
SELECT H.* FROM HEALTHMETRICS AS H
SELECT S.* FROM SALES AS S;

---------ADVANCED JOINS AND QUERYING---------------------
...
SELECT C.USER_ID,
       ...
       H.AVG_DAILY_STEPS
FROM CUSTOMERS C
INNER JOIN DEVICES D ON C.USER_ID = D.USER_ID
INNER JOIN HEALTHMETRICS H ON D.USER_ID = H.USER_ID
WHERE H.AVG_DAILY_STEPS > 1000
ORDER BY H.AVG_DAILY_STEPS DESC;
...
FROM DEVICES D
WHERE d.total_steps_recorded > 
( SELECT AVG(TOTAL_STEPS_RECORDED) FROM DEVICES)
ORDER BY d.total_steps_recorded DESC;
...
FROM 
    DEVICES D
INNER JOIN 
    CUSTOMERS C ON D.USER_ID = C.USER_ID
INNER JOIN 
    HEALTHMETRICS H ON C.USER_ID = H.USER_ID 
WHERE 
    H.USER_ID IS NOT NULL
GROUP BY 
    D.DEVICE_ID, D.DEVICE_TYPE, C.USER_ID;
...
FROM Customers c
INNER JOIN Devices D ON c.user_id = D.user_id
INNER JOIN HealthMetrics H ON c.user_id = H.user_id
WHERE 
    d.device_status = 'Active' AND
    H.avg_daily_steps > 8000 AND
    c.subscription_type IN ('Premium', 'Pro');
...
EXEC CALCULATE_TOTAL_SALES @CUSTOMERID = 1

---------

CREATE PROCEDURE SyncDeviceData
AS
BEGIN
    UPDATE [dbo].[Devices]
...
----STORED PROCEDURES AS TRANSACTIONS-----
...
SELECT 
    C.user_id,
    H.avg_daily_steps,
    H.avg_exercise_minutes,
    H.avg_sleep_hours,
	H.achievement_rate,
    dbo.CalculateWellnessScore(
        H.avg_daily_steps,
        H.avg_exercise_minutes,
        H.avg_sleep_hours
    ) AS wellness_score
FROM Customers C
JOIN HealthMetrics H ON c.user_id = H.user_id
